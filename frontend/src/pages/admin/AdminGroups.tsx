...
export default function AdminGroups() {
  ...
  const loadData = async () => {
    try {
      const { data: oData } = await supabase.from('organizations').select('id, name').order('name');
      if (oData) setOrgs(oData);
  ...
  const createGroup = async (e: React.FormEvent) => {
    ...
    try {
      await supabase.from('groups').insert([{ name: newGroupName.trim(), org_id: newGroupOrgId }]);
      setNewGroupName('');
      setNewGroupOrgId('');
      loadData();
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '0.5rem' }}>Groups Management</h1>
          <p style={{ color: 'var(--text-muted)' }}>Manage functional groups and teams across all organizations.</p>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '2rem' }}>
        ...
        <form onSubmit={createGroup} style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          ...
          <input 
            ...
            style={{ flex: 1 }}
          />
          <button type="submit" className="btn btn-primary" disabled={!newGroupName.trim() || !newGroupOrgId}>
            Create Group
          </button>
        </form>
      </div>
            ...
            <tbody>
              {groups.map(group => (
                <tr key={group.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <span className="badge badge-medium" style={{ background: '#F3E8FF', color: '#9333EA' }}>
